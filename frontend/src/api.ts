...
export const roomApi = {
  ...
  incomingPeerProbes: (roomID: number, purpose: 'ping' | 'game' = 'ping', sessionKey = '') => request<{ probes: PeerProbe[] }>(`${route(runtimeConfig.apiRoomPeerProbesPath, roomID)}/incoming?purpose=${purpose}&session_key=${encodeURIComponent(sessionKey)}`),
  peerProbe: (roomID: number, probeID: number, sessionKey = '') => request<{ probe: PeerProbe }>(`${route(runtimeConfig.apiRoomPeerProbesPath, roomID)}/${probeID}?session_key=${encodeURIComponent(sessionKey)}`),
  answerPeerProbe: (roomID: number, probeID: number, localDescription: string, sessionKey = '') => request<{ state: string }>(`${route(runtimeConfig.apiRoomPeerProbesPath, roomID)}/${probeID}/answer`, { method: 'POST', body: JSON.stringify({ local_description: localDescription, session_key: sessionKey }) }),
}
