...
const roomPreparing = ref(false)
const roomPreparationMessage = ref('')
...
const roomInfoSubtitle = computed(() => {
  ...
  if (directCandidateStatus.value === 'relay-only') return `${activeRoomName.value} · 未发现直连候选 · 仅使用中继`
...
async function resetMatchTransport() {
  gamePeerTasks.clear()
  gamePeerTransactions.clear()
  activeGamePeerIp = ''
  activeGamePeerTransaction = ''
...
async function prepareRoomTools(lease: Lease, epoch: number): Promise<'ready' | 'relay-only'> {
  const desktopApi = desktop()
...
async function configureGamePeerOnce(logicalIp: string, transactionKey: string, epoch: number) {
  ...
  try {
    ...
    // Keep the next slot warm even when signaling fails before a remote SDP
    // is installed, so the next match can rotate to a clean agent.
...
function candidateCount(description: string) {
...
onMounted(() => {
  ...
  removeTransportListener = desktop()?.onTransportChange?.((status) => { gameTransportSummary.value = status.summary })
  void restoreSession()
})
onBeforeUnmount(() => {
  stopLeaseHeartbeat()
