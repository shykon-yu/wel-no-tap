...
/* Keep the BCrypt hash object thread-local so packet authentication does not
   serialize the Hook's receive and send paths. */
...
static int welnpt_auth_digest(welnpt_auth_context *context, const char *packet,
    ...
    if (context->object_length <= sizeof(g_welnpt_auth_object)) object = g_welnpt_auth_object;
    else {
